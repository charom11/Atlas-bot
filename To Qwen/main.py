...
def check_fibonacci_setup(df, symbol="XRPUSDT"):
    ...
    try:
        ...
        # 1. Bullish Retracement into 0.618 - 0.886 Fibonacci Zone
        if is_uptrend and last_sh[0] > last_sl[0]:
            ...
            if (curr_l <= fib_0618) and (curr_p >= (fib_1000 - 0.20 * atr_val)):
                entry_p = entry_1
                ...
                risk = entry_p - sl_p
                # Blended target reward across TP1 (50% scale-out) and TP2 (50% expansion)
                ...
                return {
                    ...
                    's_low': s_low,
                    'impulse': impulse,
                    'desc': f"📐 Fib 0.618-0.886 Long | E1(0.618): ${entry_1:.4f} | E2(0.786): ${entry_2:.4f} | E3(0.886): ${entry_3:.4f} | TP1: ${tp1_p:.4f} | SL: ${sl_p:.4f}"
                }

        # 2. Bearish Retracement into 0.618 - 0.886 Fibonacci Zone
            ...
            if (curr_h >= fib_0618) and (curr_p <= (fib_1000 + 0.20 * atr_val)):
                entry_p = entry_1
                sl_p = fib_1000 + (0.50 * atr_val)
                tp1_p = s_low
                ...
                return {
                    ...
                    's_low': s_low,
                    'impulse': impulse,
                    'desc': f"📐 Fib 0.618-0.886 Short | E1(0.618): ${entry_1:.4f} | E2(0.786): ${entry_2:.4f} | E3(0.886): ${entry_3:.4f} | TP1: ${tp1_p:.4f} | SL: ${sl_p:.4f}"
                }
...
def manage_active_positions_breakeven():
    ...
    try:
        ...
        for p in positions:
            ...
            # --- STAGE 3: Dynamic TP3 Trailing Stop on the Final Runner (Profile C: 1.4x ATR) ---
...
class WeatherEnsembleBot:
    ...
    def evaluate_bar(self, df, symbol="XRPUSDT", active_count=0):
        ...
        # Upgrade 2: Check BTC ADX Market Volatility Regime (Profile C: 22 Threshold)
        ...
        if not self.paused and not CIRCUIT_BREAKER.circuit_tripped and active_count < self.max_active_positions:
            # Channel 0: 📐 Objective Fibonacci 0.618-0.886 Zone Trigger (Entry 1: 0.618 | Entry 2: 0.786 | Entry 3: 0.886)
            if fib_info.get('is_setup') and fib_info.get('rr', 0) >= 1.8:
                ...
                if smc_4h_ok and core_ok:
                    ...
                    e1 = fib_info.get('entry_1', fib_info['entry_price'])
                    e2 = fib_info.get('entry_2', fib_info['entry_price'])
                    e3 = fib_info.get('entry_3', fib_info['entry_price'])
                    print(f"[FIBONACCI 0.618-0.886 AUTO-{target_side}] {symbol} E1(0.618): ${e1:.4f} | E2(0.786): ${e2:.4f} | E3(0.886): ${e3:.4f} | TP1: ${fib_info['tp1']:.4f} | TP2: ${fib_info['tp2']:.4f} | SL: ${fib_info['sl']:.4f} (R:R {fib_info['rr']:.2f}) 📐", flush=True)
                elif not core_ok:
            ...
            # Channel 1: 31-Model Quant Consensus + Pillar Validation
                ...
                if not pillar_ok:
                    ...
                    if ob_ok and funding_ok and smc_4h_ok and of_ok and is_vol_surge and is_atr_expanded:
                        ...
                        p_sup = potato_info.get('support', 0) or 0
                        p_res = potato_info.get('resistance', 0) or 0
            ...
            # Channel 2: RSI + CCI Dual Divergence Sniper Trigger (High Conviction Lead + Confirm)
                ...
                if smc_4h_ok and core_ok:
                    action = 'BUY'
                    trade_custom_tp = potato_info.get('resistance') or (last_price + 3.2 * atr14_val)
                    trade_custom_sl = (potato_info.get('support') * 0.995) if potato_info.get('support') else (last_price - 1.5 * atr14_val)
                ...
                if smc_4h_ok and core_ok:
                    action = 'SELL'
                    trade_custom_tp = potato_info.get('support') or (last_price - 3.2 * atr14_val)
                    trade_custom_sl = (potato_info.get('resistance') * 1.005) if potato_info.get('resistance') else (last_price + 1.5 * atr14_val)
